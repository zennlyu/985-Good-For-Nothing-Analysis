...
threshold = 50                     # 聚类算法距离的阈值，这个值越高话题越集中
words_aggregation = []
...
def Pab(word, a, b):        # 求Pab=Fab/Fb
    Fb = 0
    Fab = 0
    for i in range(len(words_aggregation)):
        if b in words_aggregation[i]:
            if a in words_aggregation[i]:
                Fab += 1
        Fb += words_aggregation[i][b]
    P_ab = float(Fab / Fb)
...
for i in range(len(st)):
    if st[i] not in sentence:
        ...
        sentence[st[i]] += 1
        sum += 1
words_aggregation.append(sentence)
file.close()
